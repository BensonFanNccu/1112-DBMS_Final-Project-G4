...
@WebServlet("/ChangeEmailPage")
public class ChangeEmailPage extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
	    ...
	    if(!username.equals(reusername)) {
	    	out.println("<script>");
	    	out.println("alert('您的email前後輸入不一致，請重新輸入')");
	    	out.println("</script>");
	    }else {
	    	UserManager manager = new UserManager();
	    	String res = manager.changeEmail(request.getQueryString().split("=")[1], username);
